...
if ($role === 'administrador') {
    ...
    $chartType = 'doughnut';
    $chartData = ['labels' => ['Tutorias', 'Asistencias', 'Por evaluar'], 'values' => [(int) ($stats['tutorias'] ?? 0), (int) ($stats['asistencias'] ?? 0), (int) ($stats['evaluaciones_pendientes'] ?? 0)]];
}
...
<main class="container dashboard-page">
    ...
    <?php if (!$periodoNombre): ?>
        <p class="alert" role="alert">No hay una campana de tutorias activa. <?php if ($role === 'administrador'): ?>Activa una en <a href="<?= e(app_url('periodos/')) ?>">Campanas de tutoria</a>.<?php endif; ?></p>
    <?php endif; ?>

    <?php if ($role === 'administrador'): ?>
        <section class="stat-grid dashboard-stat-grid" aria-label="Resumen de la campana">
    ...
    <?php elseif ($role === 'tutor'): ?>
        <section class="stat-grid dashboard-stat-grid" aria-label="Resumen del tutor">
            <article class="stat-card stat-card-purple"><div class="stat-card-top"><span class="stat-label">Mis grupos</span><span class="stat-icon">GR</span></div><strong class="stat-value"><?= (int) ($stats['grupos'] ?? 0) ?></strong><span class="stat-caption">En la campana</span></article>
            ...
            <article class="stat-card stat-card-blue"><div class="stat-card-top"><span class="stat-label">Mi promedio</span><span class="stat-icon">EV</span></div><strong class="stat-value"><?= e(number_format((float) ($stats['evaluacion_promedio'] ?? 0), 2)) ?><small>/5</small></strong><span class="stat-caption">Valoracion recibida</span></article>
        </section>
    <?php else: ?>
        <section class="stat-grid dashboard-stat-grid" aria-label="Resumen del estudiante">
            <article class="stat-card stat-card-teal"><div class="stat-card-top"><span class="stat-label">Mis tutorias</span><span class="stat-icon">TI</span></div><strong class="stat-value"><?= (int) ($stats['tutorias'] ?? 0) ?></strong><span class="stat-caption">Grupos asignados</span></article>
            ...
            <article class="stat-card stat-card-purple"><div class="stat-card-top"><span class="stat-label">Por evaluar</span><span class="stat-icon">EV</span></div><strong class="stat-value"><?= (int) ($stats['evaluaciones_pendientes'] ?? 0) ?></strong><span class="stat-caption">Comparte tu opinion</span></article>
    ...
    <div class="dashboard-layout">
        <div class="dashboard-main-column">
            <section class="dashboard-panels">
                <article class="card chart-card">
                    <div class="section-heading"><div><span class="eyebrow">Actividad</span><h2><?= $role === 'administrador' ? 'Estado de la campana' : 'Resumen' ?></h2></div><span class="chart-period">Actual</span></div>
            ...
            <section class="card upcoming-card">
                <div class="section-heading"><div><span class="eyebrow">Agenda</span><h2>Proximas sesiones</h2></div><a href="<?= e($role === 'estudiante' ? app_url('mis-tutorias/') : ($role === 'tutor' ? app_url('mis-grupos/') : app_url('grupos/'))) ?>">Ver todo</a></div>
        ...
        <aside class="dashboard-side-column">
            ...
            <section class="card quick-action-card"><div class="section-heading"><div><span class="eyebrow">Atajos</span><h2>Acciones rapidas</h2></div><span class="shortcut-hint">1 click</span></div><div class="compact-actions"><?php if ($role === 'administrador'): ?><a href="<?= e(app_url('periodos/')) ?>"><span class="action-icon">CP</span><span><strong>Campanas</strong><small>Gestionar periodos</small></span><b aria-hidden="true">&rarr;</b></a><a href="<?= e(app_url('grupos/')) ?>"><span class="action-icon">GR</span><span><strong>Grupos</strong><small>Supervisar y demanda</small></span><b aria-hidden="true">&rarr;</b></a><?php elseif ($role === 'tutor'): ?><a href="<?= e(app_url('mis-grupos/')) ?>"><span class="action-icon">GR</span><span><strong>Mis grupos</strong><small>Asistencia y sesiones</small></span><b aria-hidden="true">&rarr;</b></a><a href="<?= e(app_url('disponibilidad/')) ?>"><span class="action-icon">DI</span><span><strong>Mi disponibilidad</strong><small>Actualizar horarios</small></span><b aria-hidden="true">&rarr;</b></a><?php else: ?><a href="<?= e(app_url('tutorias/create.php')) ?>"><span class="action-icon">SA</span><span><strong>Solicitar apoyo</strong><small>Elegir materias</small></span><b aria-hidden="true">&rarr;</b></a><a href="<?= e(app_url('mis-evaluaciones/')) ?>"><span class="action-icon">EV</span><span><strong>Evaluaciones</strong><small>Califica a tus tutores</small></span><b aria-hidden="true">&rarr;</b></a><?php endif; ?></div></section>
        </aside>
    </div>

    <section class="card dashboard-actions dashboard-all-actions"><div class="section-heading"><div><span class="eyebrow">Acceso directo</span><h2><?= $role === 'administrador' ? 'Administracion del portal' : 'Mi espacio' ?></h2></div><span class="section-hint">Todo en un solo lugar</span></div><div class="quick-links"><?php if ($role === 'administrador'): ?><a href="<?= e(app_url('periodos/')) ?>"><span>CP</span><strong>Campanas</strong><small>Periodos de tutoria</small></a><a href="<?= e(app_url('aulas/')) ?>"><span>AU</span><strong>Aulas</strong><small>Fisicas y virtuales</small></a><a href="<?= e(app_url('grupos/')) ?>"><span>GR</span><strong>Grupos</strong><small>Supervision y demanda</small></a><a href="<?= e(app_url('materias/')) ?>"><span>MA</span><strong>Materias</strong><small>Catalogo academico</small></a><a href="<?= e(app_url('reportes/campania.php')) ?>"><span>RC</span><strong>Reportes</strong><small>Metricas por campana</small></a><a href="<?= e(app_url('usuarios/')) ?>"><span>US</span><strong>Cuentas</strong><small>Acceso y estados</small></a><?php elseif ($role === 'tutor'): ?><a href="<?= e(app_url('mis-materias/')) ?>"><span>MA</span><strong>Mis materias</strong><small>Gestionar asignaturas</small></a><a href="<?= e(app_url('disponibilidad/')) ?>"><span>DI</span><strong>Disponibilidad</strong><small>Definir horarios</small></a><a href="<?= e(app_url('mis-grupos/')) ?>"><span>GR</span><strong>Mis grupos</strong><small>Asistencia y sesiones</small></a><?php else: ?><a href="<?= e(app_url('tutorias/create.php')) ?>"><span>SA</span><strong>Solicitar apoyo</strong><small>Elegir materias</small></a><a href="<?= e(app_url('mis-tutorias/')) ?>"><span>TI</span><strong>Mis tutorias</strong><small>Grupos asignados</small></a><a href="<?= e(app_url('mis-evaluaciones/')) ?>"><span>EV</span><strong>Evaluaciones</strong><small>Calificar tutores</small></a><?php endif; ?></div></section>
</main>
